...
describe('NodesService', () => {
  ...
  /**
   * Creates a mocked version of the TypeORM repository for testing
   * @template T
   * @return {*}  {jest.Mocked<Partial<Repository<T>>>}
   */

  function createMockRepository<T extends ObjectLiteral>(): jest.Mocked<
  ...
  beforeEach(async () => {
    // Create a mocked repository for the Node entity
    const mockRepo = createMockRepository<Node>();

    // Initialize the testing module with the mocked repository and service
    ...
    // Get instances of the service and the mocked repository
    service = module.get<NodesService>(NodesService);
    repo = module.get(getRepositoryToken(Node)) as jest.Mocked<
      Repository<Node>
    >;
    // repo.find to always return an empty array by default
    repo.find.mockResolvedValue([]);
    // Mocking recursive subtree loading method
    jest
      .spyOn(service as any, 'loadSubTreeChildren')
      .mockImplementation(
  ...
  afterEach(() => {
    jest.clearAllMocks(); //To clear mocks after every test to free up memory heap
  });

  // Should return the subtree when a valid parentId is provided
  describe('getSubtreeById', () => {
    ...
    it('should throw NotFoundException for invalid root id', async () => {
      ...
      await expect(service.getSubtreeByParentId('invalid')).rejects.toThrow(
        NotFoundException,
      );
    });
  });

  // Should return all root trees when id is not provided
  describe('getSubtree', () => {
  ...
  //should add or update property, if node not found it should throw notFoundException
  describe('addOrUpdateProperty', () => {
    ...
    it('should throw if node not found', async () => {
      repo.findOne.mockResolvedValue(null);
      await expect(service.addProperty('invalid', 'RAM', 16)).rejects.toThrow(
        NotFoundException,
      );
    });
  });

  //Should create a node with parent , if node not found it should throw notFoundException
  describe('createNode', () => {
